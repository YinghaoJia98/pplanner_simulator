...
class pplanner_teleop_twist_joy
{
public:
	pplanner_teleop_twist_joy()
	{
		...
		joy_sub_ = n_.subscribe("joy", 1, &pplanner_teleop_twist_joy::joyCb, this);
		tim_ = n_.createTimer(ros::Duration(0.1), &pplanner_teleop_twist_joy::timerCb, this);
...
{
	ros::init(argc, argv, "pplanner_teleop_twist_joy_node");
	pplanner_teleop_twist_joy ob;
	ros::spin();
	return 0;
}